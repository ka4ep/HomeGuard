...
public interface IServiceRecordRepository : IRepository<ServiceRecord>
{
    ...
    /// <summary>Planned records whose ServiceDate has already passed as of <paramref name="asOf"/>.</summary>
    Task<IReadOnlyList<ServiceRecord>> GetOverdueAsync(DateOnly asOf, CancellationToken ct = default);

    /// <summary>Planned records with ServiceDate in [asOf, asOf + withinDays].</summary>
